...
class PreviewService:
    ...
    async def get_or_generate_preview(
        ...
        existing_preview = await self.preview_generator.get_preview_path(
    ...
    async def _generate_urgent_preview(
        self, photo, preview_size: PreviewSize, format: str
    ) -> FileResponse:
        """Generate preview immediately for user requests."""
        try:
            # Get original file content
            original_content = await self.upload_service.get_photo_content(
                photo.id, self.db
            )
            if not original_content:
                raise HTTPException(
                    status_code=404, detail="Original photo file not found"
                )
            ...
            with tempfile.NamedTemporaryFile(
                ...
                if preview_path and preview_path.exists():
                    return self._create_file_response(
                        preview_path, photo.filename, preview_size.value, format
                    )
                else:
                    raise HTTPException(
    ...
    def _create_file_response(
        self, preview_path: Path, filename: str, size: str, format: str
    ) -> FileResponse:
    ...
    async def generate_all_previews_for_photo(self, photo_id: str) -> dict:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                results = await self.preview_generator.generate_all_previews(
                    Path(tmp_file.name), photo.id
                )
